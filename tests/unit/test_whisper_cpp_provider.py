...
def test_transcribe_passes_threads_and_gpu_flags(tmp_path: Path) -> None:
    model_file = tmp_path / "ggml-small.bin"
    model_file.write_bytes(b"dummy")
    ...
    mock_json_output = {"result": {"language": "en"}, "transcription": []}
    captured_cmds: list[list[str]] = []
    ...
    assert "-t" in whisper_cmd
    assert whisper_cmd[whisper_cmd.index("-t") + 1] == "6"
    assert "-ng" in whisper_cmd
